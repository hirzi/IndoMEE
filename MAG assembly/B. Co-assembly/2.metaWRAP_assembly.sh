...
###############################################################
#### Notes ###
# CHECKPOINTING. Note,metahit implements checkpoints, i.e. you can continue a MEGAHIT run from its last available check point (simply by resubmitting the job). This is a better alternative than using paid SL2 credits, as is free.
...
concat_reads_dir="/home/hl636/rds/rds-mobile-Gwxt74iudAM/Metagenomics_genomeBasedReference/metaWRAP/OUTPUT/CO_ASSEMBLY/1.5.CLEAN_CONCAT_READS"
...
#cat ${cleaned_reads_dir}/${pop}/${pop}*_1.fastq > ${concat_reads_dir}/${pop}/ALL_READS_1.fastq
#cat ${cleaned_reads_dir}/${pop}/${pop}*_2.fastq > ${concat_reads_dir}/${pop}/ALL_READS_2.fastq
# Run assembly
metawrap assembly -1 ${concat_reads_dir}/${pop}/ALL_READS_1.fastq -2 ${concat_reads_dir}/${pop}/ALL_READS_2.fastq -m ${mem_gb} -t $SLURM_CPUS_PER_TASK --megahit -o ${assembly_dir}/${pop}
