...
cleaned_reads_dir="/home/hl636/rds/rds-mobile-Gwxt74iudAM/Metagenomics_genomeBasedReference/metaWRAP/OUTPUT/CO_ASSEMBLY/1.5.CLEAN_READS"
assembly_dir="/home/hl636/rds/rds-mobile-Gwxt74iudAM/Metagenomics_genomeBasedReference/metaWRAP/OUTPUT/CO_ASSEMBLY/2.ASSEMBLY"
initial_bins_dir="/home/hl636/rds/rds-mobile-Gwxt74iudAM/Metagenomics_genomeBasedReference/metaWRAP/OUTPUT/CO_ASSEMBLY/4.INITIAL_BINNING"
#idx_0based=$(($IDX - 1))
#binners=(concoct maxbin2 metabat2)
#pops=(ASMDAI, BA0, BAPDW, BRUSBG, MALHUL, MALRPN, MALSUL, PBSHF) # pop order
pop=`sed -n ${IDX}p < /home/hl636/rds/rds-mobile-Gwxt74iudAM/Metagenomics_genomeBasedReference/metaWRAP/co_assembly/pop.list`
#pop="ASMDAI"
...
#cd ${cleaned_reads_dir}/${pop}
#for f in ALL_READS*.gz; do mv "$f" "${f%.gz}"; done
#cd $workdir
...
# Fo co-assembly, maxbin2 is too slow. So do concoct+metabat2 instead (you can always add maxbin2 or another binner later/separately).
#metawrap binning -o ${initial_bins_dir}/${pop} -t $SLURM_CPUS_PER_TASK -m ${mem_gb} -a ${assembly_dir}/${pop}/final_assembly.fasta --metabat2 --maxbin2 --concoct ${cleaned_reads_dir}/${pop}/${pop}*fastq
#metawrap binning -o ${initial_bins_dir}/${pop} -t $SLURM_CPUS_PER_TASK -m ${mem_gb} -a ${assembly_dir}/${pop}/final_assembly.fasta --metabat2 --maxbin2 --concoct ${cleaned_reads_dir}/${pop}/${pop}*fastq.gz # doesn't work!
#metawrap binning -o ${initial_bins_dir}/${pop} -t $SLURM_CPUS_PER_TASK -m ${mem_gb} -a ${assembly_dir}/${pop}/final_assembly.fasta --${binners[${idx_0based}]} ${cleaned_reads_dir}/${pop}/${pop}*fastq
#metawrap binning -o ${initial_bins_dir}/${pop} -t $SLURM_CPUS_PER_TASK -m ${mem_gb} -a ${assembly_dir}/${pop}/final_assembly.fasta --metabat2 --concoct ${cleaned_reads_dir}/${pop}/${pop}*fastq
metawrap binning -o ${initial_bins_dir}/${pop} -t $SLURM_CPUS_PER_TASK -m ${mem_gb} -a ${assembly_dir}/${pop}/final_assembly.fasta --maxbin2 ${cleaned_reads_dir}/${pop}/${pop}*fastq

# Rename cleaned reads fastq to original format (i.e. add .gz to filenames)
#cd ${cleaned_reads_dir}/${pop}
#for f in ALL_READS*; do mv "$f" "$f.gz"; done
#cd $workdir
